...
def _download_cifar10_manual(root="./data"):
    """
    Télécharge CIFAR-10 manuellement depuis des miroirs alternatifs
    si le serveur officiel (cs.toronto.edu) est indisponible.

    Miroirs testés dans l'ordre :
      1. https://www.cs.toronto.edu/~kriz/cifar-10-python.tar.gz  (officiel)
      2. https://huggingface.co/datasets/cifar10/...              (HuggingFace)
    """
    ...
    if os.path.isdir(dest_dir):
        print("  ✓ Archive CIFAR-10 déjà présente localement.")
        return
    ...
    for url in mirrors:
        try:
            ...
            print(f"  ✗ Échec ({e}), essai du miroir suivant...")

    if not downloaded:
        raise RuntimeError(
            "\n\n"
            "════════════════════════════════════════════════════════════\n"
            "  ERREUR : Impossible de télécharger CIFAR-10 automatiquement.\n"
            "  Tous les miroirs sont indisponibles (erreur réseau / proxy).\n\n"
            "  ► SOLUTION MANUELLE :\n"
            "    1. Téléchargez le fichier depuis l'un des liens suivants :\n"
            "       • https://www.cs.toronto.edu/~kriz/cifar-10-python.tar.gz\n"
            "       • https://huggingface.co/datasets/uoft-cs/cifar10\n"
            "    2. Placez le fichier 'cifar-10-python.tar.gz' dans :\n"
...
def _load_cifar10_from_disk(root="./data"):
    """
    Charge CIFAR-10 directement depuis les fichiers binaires .bin
    (cifar-10-batches-py) sans passer par torchvision.
    """
    import pickle, os
    ...
    X_parts, y_parts = [], []
    for i in range(1, 6):
        d = unpickle(os.path.join(base, f"data_batch_{i}"))
        X_parts.append(d[b"data"])
        y_parts.extend(d[b"labels"])

    X_train_raw = np.concatenate(X_parts, axis=0)          # (50000, 3072) uint8
    y_train     = np.array(y_parts, dtype=np.int64)

    # --- Test ---
    d_test  = unpickle(os.path.join(base, "test_batch"))
    X_test_raw = d_test[b"data"]                            # (10000, 3072) uint8
    y_test     = np.array(d_test[b"labels"], dtype=np.int64)

    # Reshape : (N, 3072) → (N, 3, 32, 32) → (N, 32, 32, 3), normalise ∈ [0,1]
    def reshape(X):
        return (X.reshape(-1, 3, 32, 32)
                  .transpose(0, 2, 3, 1)
                  .astype(np.float32) / 255.0)

    return reshape(X_train_raw), y_train, reshape(X_test_raw), y_test


def load_cifar10_numpy():
    """
    Charge CIFAR-10 et le convertit en tableaux NumPy.

    Stratégie robuste en 3 étapes :
      1. Essaie torchvision.datasets.CIFAR10 avec download=True (serveur officiel).
      2. Si le serveur officiel renvoie une erreur HTTP, tente des miroirs alternatifs
         via _download_cifar10_manual().
      3. Charge toujours depuis les fichiers binaires locaux via _load_cifar10_from_disk().
    ...
    try:
        ...
        print(f"  ✗ torchvision download échoué : {e}")
        print("  → Bascule sur téléchargement manuel depuis miroirs alternatifs...")

    # ── Étape 2 : téléchargement manuel via miroirs ──────────────────────────
    _download_cifar10_manual(root=DATA_ROOT)

    # ── Étape 3 : chargement binaire direct (sans torchvision) ──────────────
    print("  → Chargement depuis les fichiers binaires locaux...")
    X_train, y_train, X_test, y_test = _load_cifar10_from_disk(root=DATA_ROOT)
